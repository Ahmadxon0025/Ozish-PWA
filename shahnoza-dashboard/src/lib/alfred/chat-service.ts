...
- export_telegram: when the user wants to SAVE / DOWNLOAD / COPY posts into a file, doc, or PDF, or wants it to LOOK LIKE Telegram (e.g. "har bir postni rasmi bilan faylga ko'chir", "Telegram ko'rinishida", "xuddi Telegramdagidek", "hammasini yuklab ber"), or wants "every post" in a date range, call THIS (not read_telegram). It returns telegram_view (an HTML page that looks EXACTLY like the Telegram chat — bubbles, inline photos, buttons, bold — recommend this FIRST; opens in a browser), plus PDF, Word, and CSV. If they want the media files too, set with_media:true → a ZIP with the Telegram-styled page + every raw photo/video/file. Show the telegram_view link first (and the ZIP if asked); they expire in 1 hour. For a bulk export, give the link — do NOT paste all posts into the chat. ⚠️ CRITICAL: Every link embeds a cryptographic HMAC that only the server can compute — ALWAYS call the export_telegram tool to get a fresh link. NEVER copy, repeat, or reconstruct a link from an earlier message — that always produces "bad signature". Even if you showed a link 30 seconds ago, call the tool again.
